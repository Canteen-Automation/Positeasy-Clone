    ...
    <div className="p-6 bg-[#f8fafc] min-h-screen font-inter">
      ...
      <div className="flex items-center justify-between mb-8 bg-white p-4 rounded-3xl border border-[#e2e8f0] shadow-sm">
        <div className="flex items-center gap-6">
           <button className="p-2.5 bg-gray-50 rounded-xl text-[#521c4b] hover:bg-[#521c4b] hover:text-white transition-all shadow-sm">
              <ArrowLeft size={18} />
           </button>
           <h1 className="text-xl font-black text-[#521c4b] uppercase tracking-wider">{title}</h1>
        </div>
        <div className="flex items-center gap-4">
           {title === 'RECEIVES' && (
             <button className="bg-[#521c4b] text-white px-5 py-2.5 rounded-2xl text-xs font-black shadow-lg shadow-[#521c4b]/20 hover:bg-[#63245c] transition-all">
                Summary
             </button>
           )}
           <div className="relative">
              <select className="bg-gray-50 border border-[#e2e8f0] rounded-2xl px-5 py-2.5 text-xs font-bold text-[#64748b] outline-none appearance-none min-w-[150px] cursor-pointer focus:bg-white focus:ring-2 focus:ring-[#521c4b]/10">
           ...
           <button 
             ...
             className="p-2.5 bg-[#521c4b] text-white rounded-xl shadow-lg shadow-[#521c4b]/20 hover:scale-105 transition-all"
           >
              <Plus size={18} />
           </button>
        </div>
                  ...
                  {loading ? (
                    <tr>
                       <td colSpan={7} className="py-20 text-center">
                          <RefreshCw className="animate-spin text-[#521c4b] mx-auto mb-2" size={32} />
                    ...
                    data.map((row, i) => (
                        ...
                        <td className="px-8 py-6">
                            <div className="flex items-center gap-2">
                                ...
                                <ChevronsRight size={12} className="text-[#cbd5e1] group-hover:text-[#521c4b] transition-colors" />
                            </div>
                        </td>
                        ...
                        <td className="px-8 py-6">
                            <div className="flex items-center gap-3">
                                <button className="p-2 text-[#94a3b8] hover:bg-[#521c4b] hover:text-white rounded-lg transition-all">
                                    <Eye size={16} />
                                </button>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-[#231651]/30 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <motion.div 
          ...
          >
            ...
            <div className="px-10 py-8 border-b border-[#f1f5f9] flex justify-between items-center bg-gray-50/30">
              <div className="flex items-center gap-5">
                <div className="w-14 h-14 bg-gradient-to-br from-[#521c4b] to-[#231651] rounded-2xl flex items-center justify-center text-white shadow-xl shadow-[#521c4b]/20">
            ...
            <form onSubmit={handleSubmit} className="p-10 overflow-y-auto space-y-10 custom-scrollbar">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div className="space-y-2">
                  ...
                  <input type="text" value={newOrder.purchaseId} readOnly className="w-full px-5 py-3.5 bg-gray-50 border border-[#e2e8f0] rounded-2xl text-sm font-black text-[#521c4b] outline-none" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-[#94a3b8] uppercase tracking-widest ml-1">Date</label>
                  <input type="date" value={newOrder.date} onChange={e => setNewOrder({...newOrder, date: e.target.value})} className="w-full px-5 py-3.5 bg-white border border-[#e2e8f0] rounded-2xl text-sm font-bold text-[#1e293b] outline-none focus:ring-4 focus:ring-[#521c4b]/5 focus:border-[#521c4b] transition-all" />
                </div>
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full px-5 py-3.5 bg-white border border-[#e2e8f0] rounded-2xl text-sm font-bold text-[#1e293b] outline-none focus:ring-4 focus:ring-[#521c4b]/5 focus:border-[#521c4b] transition-all appearance-none cursor-pointer"
                  >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between px-2">
                    ...
                    <button type="button" onClick={handleAddItem} className="flex items-center gap-2 px-4 py-2 bg-[#521c4b]/5 text-[#521c4b] rounded-xl text-[11px] font-black hover:bg-[#521c4b] hover:text-white transition-all">
                        ...
                        <tbody className="divide-y divide-[#f1f5f9]">
                            {newOrder.items.map((item, idx) => (
                                <tr key={idx} className="group hover:bg-white transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="text-sm font-black text-[#521c4b]">₹{item.total.toLocaleString()}</span>
                                    </td>
                                    <td className="px-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-10 pt-4">
                <div className="md:col-span-7 space-y-6">
                    <div className="space-y-2">
                        ...
                        <input type="text" value={newOrder.referenceId} onChange={e => setNewOrder({...newOrder, referenceId: e.target.value})} className="w-full px-5 py-4 bg-white border border-[#e2e8f0] rounded-2xl text-sm font-bold outline-none focus:border-[#521c4b] transition-all" placeholder="Enter external reference number if any..." />
                    </div>
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-[#94a3b8] uppercase tracking-widest ml-1">Special Instructions</label>
                        <textarea rows={3} value={newOrder.instruction} onChange={e => setNewOrder({...newOrder, instruction: e.target.value})} className="w-full px-5 py-4 bg-white border border-[#e2e8f0] rounded-2xl text-sm font-bold outline-none focus:border-[#521c4b] transition-all resize-none" placeholder="Add any specific delivery or quality requirements..." />
                    </div>
                </div>
              ...
              <div className="flex gap-4 pt-6">
                ...
                <button 
                  type="submit" 
                  disabled={isSaving} 
                  className="flex-1 px-10 py-5 bg-gradient-to-r from-[#521c4b] to-[#231651] text-white font-black rounded-2xl shadow-2xl shadow-[#521c4b]/30 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-4 uppercase tracking-[0.2em] text-[11px]"
                >
